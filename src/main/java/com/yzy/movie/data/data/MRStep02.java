package com.yzy.movie.data.data;
...
public class MRStep02 {
	...
	public static void main(String[] args) throws Exception {
		...
		//先获取数据，存储在List中
		Path src2 = new Path("/douban2/step_01/part-r-00000");
		...
		Path src = new Path("/douban2/step_01");
		Path dst = new Path("/douban2/step_02");
		fs.delete(dst, true);
		...
		job.setMapOutputValueClass(MyMovie.class);
	...
	public static class MyMapper extends Mapper<Object, Text, Text, MyMovie>{
		...
		protected void map(Object key, Text value, Context context)
			...
			for(int i=0; i<scoreList.size(); i++) {
				...
				sum = Math.round(sum*1000000)/10000.0;//保留小数点后4位
				outKey.set(a[0]);
				MyMovie m = new MyMovie(c[0], sum);
				context.write(outKey, m); 
			}
		}
	}
	
	public static class MyReducer extends Reducer<Text, MyMovie, Text, Text>{
		private Text outKey = new Text();
		private Text outValue = new Text();
		
		@Override
		protected void reduce(Text key, Iterable<MyMovie> values, Context context) 
				throws IOException, InterruptedException {
			List<MyMovie> list = new ArrayList<>();
			StringBuffer s = new StringBuffer();
			for(MyMovie m : values) {
				list.add(m);
			}
